...
#define MOD(x, y) ((x)<0 ? ((x)%(y)+(y)) : ((x)%(y)))
  ...
  return logicState->previousCAArray[MOD(posX, logicState->sizeCAArray) * logicState->sizeCAArray +
                                     MOD(posY, logicState->sizeCAArray)];
}

void nextStep(LogicState *logicState) {
  for (int i = 0; i < logicState->sizeCAArray; ++i) {
    for (int j = 0; j < logicState->sizeCAArray; ++j) {
      logicState->previousCAArray[logicState->sizeCAArray * i + j] = logicState->currentCAArray[
            logicState->sizeCAArray * i + j];
    }
  }
